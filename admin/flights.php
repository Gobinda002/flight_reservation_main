...
function deleteFlight(flightId) {
    if (!confirm("Are you sure you want to permanently delete this flight?")) {
        return;
    }
    ...
    fetch("delete_flight.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: "flight_id=" + flightId + "&action=delete"
    })
    .then(response => response.text())
    .then(data => {
        if (data === "success") {
            // Remove both the flight row and its cancel form if present
            document.getElementById("row-" + flightId).remove();
            const cancelForm = document.getElementById("cancel-form-" + flightId);
